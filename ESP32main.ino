  ...
  while(true){
    ...
    for(int i=0; i<Nwave; i++) //Bucle que lee muestras de audio
    {
      ...
      if (i2s_pop_sample(I2S_NUM_1, (char*)&mic_sample, portMAX_DELAY) == 4) {
        ...
        wave[i] = mic_val;
        mic_sample = (mic_sample | 0x8000);
        mic_sample = (mic_sample & 0xFFFF);
...
//   Serial.println("------------------->> Entra FFT");
  ...
  while(true){
   ...
   for(int i=0; i<Nwave; i++) //Bucle que lee 1024 muestras de audio
    {
      vReal[i] = wave[i];
    ...
    if(localState == AUDIO){
      ...
      /*if(numTramasGuardadas <= 3){
       ...
       if( xSemaphoreTake( dataSemaphore, portMAX_DELAY ) == pdTRUE )
         ...
         //audioToSend = "";
         savedAudio.remove(0, indFin); //quitar 2 tramas
         ...
         if((numTramasEnviadas >= tramas10Segundos)){
           concat = 0;
         }
    ...
    while(true){
        ...
        if (client.connect(raspip.c_str(), port)) {
            ...
            if( xSemaphoreTake( stateSemaphore, portMAX_DELAY ) == pdTRUE )
            {
                ...
                switch (state_env) {
                    case VOLUME:
                        client.println(makeHTTPrequest("POST","/volume","application/json",data, getDelayLocalization() ,EndOfFile));
                    ...
                    break;
                    default:
                        client.println(makeHTTPrequest("POST","/error","application/json",data , getDelayLocalization(), EndOfFile));
                        state_env=IDLE;
                        raspiListening=false;
                    break;
                    }
